...
For **carry (YES)** plans, `ev_breakdown` always includes: `mode: "carry"`, `carry_roi_pct`, `hold_to_resolution`, `time_to_resolution_days`. **`carry_roi_pct`** is the **annualized** carry ROI (APR): raw hold-to-resolution ROI scaled to a 365-day year; the strategy gates candidates on this APR band (config `roiMinPct` / `roiMaxPct`). **`carry_roi_raw_pct`** (optional) is the raw hold-to-resolution ROI in percent, for debugging only.
